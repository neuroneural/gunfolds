...
from gunfolds.utils.clingo import clingo_old
...
def drasl(glist, capsize, timeout=0, urate=0, weighted=False, scc=False,  dm=None,
    ...
    if not isinstance(glist, list):
        glist = [glist]
    return clingo_old(drasl_command(glist, max_urate=urate, weighted=weighted,
...
def rasl(g, capsize, timeout=0, urate=0, pnum=None):
    ...
    :rtype: dictionary
    """
    return clingo_old(rasl_command(g, urate=urate), capsize=capsize, convert=rasl_jclingo2g, timeout=timeout, pnum=pnum)